...
libraries(c("readxl", "readr", "plyr", "dplyr", "ggplot2", "png", "tidyverse", "reshape2", "scales", "viridis", "rgdal", "officer", "flextable", "tmaptools", "lemon", "fingertipsR", "PHEindicatormethods", 'jsonlite', 'readODS'))
...
# Understanding the population that a hospital supports is an important part of service planning and healthcare provision. Hospital provider trusts do not have geographically defined boundaries for their client population nor do they have all encompassing lists of registered patients. Public Health England (PHE) has published modelled estimates of the catchment populations for hospital provider trusts in England. These experimental statistics show catchment populations and give estimates for the number of people who are using each hospital trust or have the potential to do so. They may also be referred to as the ‘effective population’ or ‘population at risk’. Catchment populations can be used to generate hospital catchment areas which are notional geographical areas from which the clients of a particular acute trust are drawn. Acute trusts currently use varying methods to define the population they serve, such as  Patient Flow Modelled Estimates, CCG Derived or Travel Time. PHE published modelled estimates of the catchment populations for Acute trusts in England using the patient flow method.
...
Monthly_cc_sitrep <- Monthly_cc_sitrep_1 %>%
  ...
  mutate(Percentage_adult_beds_available = adult_beds_available / adult_beds_open) %>% 
...
Daily_cc_sitrep <- read_csv(paste0(github_repo_dir, '/daily_cc_adult_bed_occupancy.csv')) %>% 
  ...
  spread(Metric, Beds) %>% 
  mutate(Occupied = as.numeric(`CC Adult Occ`)) %>% 
  mutate(Open = as.numeric(`CC Adult Open`)) %>% 
